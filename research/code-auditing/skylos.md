...
- Use the MCP `verify_change` result as one evidence source in a coding-agent verification gate. In this repository the candidate gate locations are `plugins/development-harness/skills/execution/SKILL.md`, whose post-task gate declares format, lint, typecheck, and test checks and no static-analysis check, and `plugins/development-harness/agents/feature-verifier.md`, which has no deterministic symbol-or-API proof step. Suitability depends on the target repository's languages and accepted `incomplete` policy. [Source: README](https://github.com/duriantaco/skylos/blob/main/README.md)
- Use the `-a` combined scan as deterministic detection behind the changed-file review agents `plugins/development-harness/agents/reviewer-security.md` and `plugins/development-harness/agents/reviewer-quality.md`, whose documented methods for secrets, injection, and dead code are pattern-based. The per-perspective scope split those two agents already declare must be preserved when mapping findings. [Source: README](https://github.com/duriantaco/skylos/blob/main/README.md)
- Use `.skylos/agent-test.yml` as a deterministic behavioral-contract format for agents whose tool choices, refusals, or source references are testable. This repository already declares each of those three axes separately — `plugins/development-harness/skills/dispatch-contract/SKILL.md` for declared tool scope, `plugins/development-harness/skills/subagent-contract/SKILL.md` for the `STATUS: DONE`/`STATUS: BLOCKED` refusal state, and `plugins/development-harness/skills/review-verdict-contract/references/verdict-schema.md` for `SKIP` plus source-reference conservation — so this is an extension point, not a greenfield format. This is an integration proposal, not a claim that Skylos evaluates semantic correctness. [Source: Agent Behavior Testing](https://github.com/duriantaco/skylos/blob/main/docs/agent-behavior-testing.md)
