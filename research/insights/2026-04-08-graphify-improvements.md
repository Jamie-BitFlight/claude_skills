...
| Multi-platform skill format | Already covered locally via multi-ecosystem frontmatter preservation (`rules/frontmatter-requirements.md`, "Multi-Ecosystem Frontmatter Preservation" section). The `mcp:` field for OpenCode is documented there (`rules/frontmatter-requirements.md` line 43). The "ecosystem registry" this justification also cited, at `plugins/plugin-creator/scripts/ecosystem_registry.py`, was present when this entry was written — added 2026-03-06 by commit `04865ac8f` ("feat(plugin-creator): add multi-ecosystem awareness for OpenCode skill compatibility") — and was deleted on 2026-05-04 by PR #2116 (commit `8257ea719`), which moved skill-creator's eval agents to plugin-level `agents/`. The citation was accurate on this entry's date and no longer resolves against the current tree; nothing replaced that half of the coverage claim, so verify it against `rules/frontmatter-requirements.md` alone. |
