...
| Multi-agent coordination (MsgHub message routing) | Already covered -- swarm-operations SKILL.md provides TeamCreate, SendMessage (direct + broadcast), TaskCreate/TaskUpdate with dependency management, shutdown sequences, and inbox-based message routing. Functionally equivalent to MsgHub with managed participants. **[Refuted — the skill(s) cited here were retired in PR #3422 (`4e1e73bd6`) and are absent from the current tree; see the removed-skill note at the top of this file.]** |
