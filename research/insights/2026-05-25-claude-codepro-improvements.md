...
| Modular Rules System with `/context` verification | Already covered — `rules/*.md` files + `CLAUDE.md` Required Reading sections already implement a modular rules system. `/context` is a Claude Code built-in command for context verification and is not skill-owned. Adding skill documentation about a built-in verification command would duplicate behavior the user already has access to natively. |
