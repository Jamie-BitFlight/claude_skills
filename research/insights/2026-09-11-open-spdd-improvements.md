...
| Cross-tool compilation — one methodology emitted into each tool's native command format, with marker-file auto-detection (research entry, Relevance item 2) | Already tracked. 29 open backlog items cover it per plugin (e.g. #3481 development-harness, #3491 plugin-creator, #3496 scientific-method), against the `claude-code` / `codex` / `hermes` / `kimi` target set recorded in the generated `harness_compatibility.json` view after running `uv run --script scripts/generate_harness_compatibility.py`. |
