...
| Separated concerns (data collection, training, inference) | Already covered. The local SAM workflow implements clear three-phase separation: planning (`/add-new-feature`), execution (`/implement-feature`), and quality gates (`/complete-implementation`). Each phase is a separate skill — `plugins/development-harness/skills/add-new-feature/SKILL.md`, `plugins/development-harness/skills/implement-feature/SKILL.md`, `plugins/development-harness/skills/complete-implementation/SKILL.md`. The architecture document originally cited here (`.claude/rules/local-workflow.md`) was deleted 2026-03-29 (commit 18f93b5c6) with no successor document; the separation is now observable only from the skill files themselves. |
