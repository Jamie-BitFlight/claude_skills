...
| Dependency isolation via uv | Already covered — the local repo uses `uv` as the primary package manager (documented in [rules/uv-run-required.md](../../rules/uv-run-required.md)). All Python scripts run via `uv run`. |
