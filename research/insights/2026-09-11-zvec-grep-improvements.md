...
> **Superseded citations:** `./plugins/python3-development/skills/semantic-code-search/SKILL.md`, cited three times by Improvement 1 as its Local system and in two of its Measurable signals, was deleted in commit `f3ffbeb55` (#3836, 2026-09-24) when `python3-development` was retired into `python-engineering`. The plugin name is not the problem: the successor that carries the name, `python-engineering:semantic-code-search`, is an **agent**, not a skill, and it disclaims the retrieval path Improvement 1 is about — "This agent searches with Grep and Glob, not by embedding similarity", and "Searching by meaning alone, with no identifier or pattern to anchor on, needs an indexed search backend this plugin does not ship". No `.mcp.json` or `cocoindex` reference survives anywhere under `plugins/python-engineering/`. The paths are left dead rather than renamed — see the note under Improvement 1's own heading.
...
> **Superseded — do not action.** The two-branch router this improvement critiques no longer exists, and neither does the second branch a third branch would have combined. The 18-line skill was accurate when read: line 14 was verbatim `Prefer Grep/Glob when you know exact identifiers, filenames, or string literals. Prefer semantic search when you know what the code *does* but not what it's *called*.` Both Measurable signals are now unrunnable — against the successor agent, `grep -c 'Prefer'` returns 0 and `grep -n 'call-chain\|data-flow\|both'` returns no match, and there is no file in which a reconciliation rule could be written. The hybrid-retrieval premise would have to be re-aimed at whatever owns semantic search now: CocoIndex survives as the repo-level `/ccc` skill (`.claude/skills/ccc`), which `dh:codebase-analyzer` still calls — so this improvement's own "Note on partial coverage" below remains true, and that agent, not a retired plugin skill, is where a combined-path routing rule would have to live. That is a new design question, not the change described here.
