...
|---|---|
| Streaming JSON parser with 8KB ring buffer | Architectural mismatch — Claude Code agents run in environments with megabytes/gigabytes of memory. The constraint that motivates ring-buffer streaming (400KB SRAM on ESP32) does not exist in any local file under `./plugins/` or `./.claude/skills/`. Pattern is incompatible with the local architecture. |
