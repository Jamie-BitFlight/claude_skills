...
`sam_schema/cli.py plan sam-task-create` creates a real GitHub issue and sub-issue link as soon as it is invoked, including under `CliRunner`. It has no dry-run or `--plan-dir` option, so with `GITHUB_TOKEN` set it writes to GitHub. `sam-tasks`, `sam-task-status` and `sam-ready-tasks` also hit the live backend.
