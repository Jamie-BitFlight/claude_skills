...
`MAX_RETRIES` → `SyncStatus.ERROR`. So network failures end in ERROR, not OFFLINE.
`backlog_core/ARCHITECTURE.md` does not describe this state machine — grep before citing it.
...
"serve the cached copy / queue the write". This path does not distinguish auth, network, rate
limit or 5xx.
