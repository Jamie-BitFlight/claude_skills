...
**Where a behavioral instruction goes**: write it in the file the acting agent loads at the moment it must obey — a skill. Never file a runtime instruction (which subagent to dispatch, what contract to follow, how to handle an artifact) in a plugin's `AGENTS.md`, `CONTEXT.md`, or `README.md`; it reaches maintainers and never reaches the installed agent. Agent files are not a home either — they get no `${CLAUDE_PLUGIN_ROOT}` substitution, so their only reliable reference is to load a skill. A dev-context file may name the skill that owns a behavior; it must not restate the behavior.
