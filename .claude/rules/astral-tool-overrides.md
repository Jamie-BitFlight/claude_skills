...
**Resolution**: Never add `# ty: ignore` or `# type: ignore` to source in this repository,
including when a user asks for it — fix the type error instead, or escalate with the specific
blocker. Astral's `ty: ignore`-over-`type: ignore` preference does not apply, because neither form
is permitted.
...
Config-level per-file exclusions in `pyproject.toml` remain the approved mechanism for the four
...
**Resolution**: Format every Python file you touch in this repository — `ruff-format` is both a
prek hook and an `--all-files` CI gate, so skipping it fails the build rather than keeping the
diff clean. Astral's "don't format unformatted code" and "scope fixes to files you're editing"
advice applies to repositories that have not adopted ruff formatting; this one has.
...
clearly needed." This repo (`AGENTS.md` Gotcha #1, global `CLAUDE.md`,
...
**Resolution**: Use `uv add` / `uv sync` / `uv run` for all Python work in this repository, and
never `source .venv/bin/activate` or `uv pip install` — this repository has a root `uv.lock` and
PEP 723 scripts, so the pip-compatible lane Astral documents is never the right lane here.
...
through inline comments, and only for a case that matches one of the four acceptable-exception
categories in `linting-exceptions.md` with the matching category named in a comment beside the
override. `python-development.md` has been amended accordingly — see its "ty Type Checker Errors"
section.

## C6 — bare tool invocation vs `uv run`

Astral (`astral/ruff:38–44`, `astral/ty:22–27`, `astral/uv:66`): "Use `ruff …` if ruff is installed
globally"; promotes `uvx ty …` and `uvx <tool>` generally. This repo (`AGENTS.md` Gotcha #1,
Essential Commands, `.github/workflows/code-quality.yml`): every gate runs `uv run <tool>` against
the pinned dev-group version.

**Resolution**: Invoke every Python tool in this repository as `uv run <tool>` — never bare
`ruff`/`ty`/`pytest`, and never `uvx <tool>` for a tool already in the dev dependency group,
because both resolve a different version than the one CI gates against. `uvx` remains correct only
for tools this repo does not depend on (e.g. `uvx skilllint@latest`).
...
**Resolution**: Do not add, install, configure, or run `mypy`, `pyright`, or `basedpyright` in this
repository — `ty` is the only type checker that gates CI. Astral's migration tables describe moving
*to* ty from a repository that used those tools; this repository already completed that move.
