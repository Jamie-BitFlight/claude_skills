...
For complete, ready-to-use `gh` commands with the correct `-R` flag for this repository, read `.claude/skills/gh/gh-examples.md` — generated by `setup_gh.py` at install time and regenerated on each run to reflect the current repo's owner/repo slug.

```bash
# Read the generated examples file for this repo's ready-to-use commands
