...
3. Run every gate in the order the rubric lists them, each over the files that gate itself names — Gate 1 reads the entry and probes its source URL, Gate 2 reads the entry's "Relevance to Claude Code Development" section plus the analysis files, and then reads this repository's own files to settle each claim, and Gate 3 reads only the analysis files, adjudicating their own reasoning. The rubric scores nothing outside those gates: depth, prose, per-section confidence, capability figures, formatting, and cross-reference symmetry are not defects here, and reporting them as defects manufactures work the repo has already automated elsewhere. Scoring an analysis file against the entry's required sections manufactures defects the same way. Everything each gate needs is inside `.claude/skills/research-curator/` — never reach for another plugin, command, or skill to run one.
4. A gate you could not run is recorded `NOT RUN` with the reason; it is never a pass, and it is never omitted from the report. A gate whose files are legitimately absent for this entry — no analysis file was written — is `NOT RUN` with that as the reason, not a defect against the entry.
   A source URL you could not reach is neither: run the rubric's probe, record its outcome as `UNVERIFIED` with the verbatim write-out line, and carry on through the rest of Gate 1 and through Gate 2. A sandbox with no egress returns `000` for every URL, so treating an unreachable host as a dead one would fail every entry the moment the network does. Never substitute a WebFetch or an MCP fetch for the probe to turn `UNVERIFIED` into a verdict.
...
- Write to any file while running `--review`, the entry under review included. `--review` records defects; `--fix` and `--rerun` apply them
