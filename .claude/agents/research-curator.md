...
3. Run both gates in the order the rubric lists them, each over the files that gate itself names — Gate 1 reads the entry and fetches its source URL, Gate 2 reads the entry's "Relevance to Claude Code Development" section plus the analysis files, and then reads this repository's own files to settle each claim. The rubric scores nothing outside those two gates: depth, prose, per-section confidence, capability figures, formatting, and cross-reference symmetry are not defects here, and reporting them as defects manufactures work the repo has already automated elsewhere. Scoring an analysis file against the entry's required sections manufactures defects the same way.
