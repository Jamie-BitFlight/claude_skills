...
5. **Quoted line contains the search term** — the entry's Relevance items quote a line as evidence for a term. Check that the quoted line actually contains that term. A quote that does not is evidence about something else, and is a defect no matter how real the path is: a GUI `widget` anchored to a tmux menu widget, an SDL2 `simulator` anchored to an iOS Simulator.
...
- An **absence anchor** passes when you re-run its search command yourself and get the same zero. Its `→ 0 matches` is true of most repositories, so it never satisfies the would-be-false-elsewhere test — re-running the command is what makes it a finding rather than a claim, and running it is the check. An absence anchor whose command you did not re-run is `NOT RUN`, not a pass. An absence anchor whose command now returns matches is a defect: the entry asserts nothing exists where something does.
