...
5. **Quoted line contains the matched term** — each Relevance item quotes a line as evidence for a capability that was searched under a narrow and a broader term. Check that the quoted line contains whichever of the two produced the match list (the anchor record's `Term matched:` field names it). A quote containing neither is evidence about something else, and is a defect no matter how real the path is: a GUI `widget` anchored to a tmux menu widget, an SDL2 `simulator` anchored to an iOS Simulator.
