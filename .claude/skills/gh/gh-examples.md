<!-- Generated by setup_gh.py — do not edit directly. Edit gh-examples.md.template instead. -->

# GitHub CLI (gh) — Command Examples

All commands in this file use `Jamie-BitFlight/claude_skills` as the repository slug.
This file is generated from `gh-examples.md.template` by `setup_gh.py` at install time.

**RULE: Pass `-R` (or `--repo`) on EVERY `gh` command** in proxy environments where git
remotes point to a local proxy instead of `github.com`.
