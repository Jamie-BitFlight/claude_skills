...
5. Keep delegation grounded in current session facts only — an ECOSYSTEM CONTEXT slot for session-specific state (parallel agents on the same files, unmerged branches) that cannot be discovered by reading the repo, kept separate from durable conventions.
