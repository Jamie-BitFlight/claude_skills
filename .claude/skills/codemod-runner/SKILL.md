...
Do not commit per-batch — commit after all batches complete and verification passes.
...
For codebases with thousands of files, use Pattern 6 from `/swarm-patterns` to coordinate parallel workers across file batches. Each worker owns one batch file, runs the codemod + idempotency check, and reports its completion count to the team lead before the final commit.
