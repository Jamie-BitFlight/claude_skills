...
def register_mutation_commands(  # ruff: ignore[complex-structure,too-many-statements] - declarative Typer registration
    app: typer.Typer, target_resolver: TargetResolver, provider_resolver: ProviderResolver
) -> None:
    """Register cycle validation and authorized mutation commands.
    ...
    def reply(
        ...
        result = provider_resolver(target).act(target, action, command_timeout=provider_timeout_seconds)
    ...
    @app.command(name="approval-state")
    def approval_state(
        pr: Annotated[int, typer.Option()],
        input_id: Annotated[str, typer.Option()],
        snapshot_file: SnapshotFile,
        state_file: StateFile,
        approved: Annotated[bool, typer.Option("--approved/--unapproved")],
        github: GithubOption = None,
        provider: ProviderOption = None,
        repo: RepoOption = None,
        host: HostOption = None,
        ...
        """Set an authorized provider approval state.

        Args:
            pr: Change-request number.
            input_id: Canonical inbound input binding the authorized cycle.
            snapshot_file: Complete canonical snapshot JSON.
            state_file: Complete review-cycle JSON.
            approved: Requested authenticated-actor approval state.
            github: Legacy explicit GitHub repository.
            provider: Explicit provider selection.
            repo: Provider repository path.
            host: Bare provider hostname.
        ...
        """
        target = target_resolver(provider, repo, host, github, pr, command_timeout=provider_timeout_seconds)
        action, _cycle = authorized_action(
            target, snapshot_file, state_file, input_id, ApprovalStateAction(approved=approved)
        )
        result = provider_resolver(target).act(target, action, command_timeout=provider_timeout_seconds)
        typer.echo(json.dumps(result.raw))
