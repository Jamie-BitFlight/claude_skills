...
7. Once all current threads are resolved, re-check for new reviews using the `watch` subcommand — it blocks its own process on an internal polling loop and returns as soon as new activity appears or its timeout elapses, so no separate resumption mechanism (scheduling, wakeups) is needed to check again later:
   ...
   How to run it depends on which entity you are — determine this from an observable fact, not a self-reported role: is `ScheduleWakeup` present in your own current tool list right now? `ScheduleWakeup` sits on the same subagent tool denylist as `Agent`, `AskUserQuestion`, `EnterPlanMode`, and `ExitPlanMode` (source: <https://code.claude.com/docs/en/sub-agents.md> § Available tools) — the harness strips all of them from a Task/Agent-dispatched sub-agent's tool list regardless of what a dispatch prompt says about the agent's role, so its presence or absence reliably distinguishes the live interactive session from a bounded dispatch. Confirmed for Task/Agent-dispatched sub-agents; unconfirmed for TeamCreate teammates, `kage-bunshin` peers, Remote Control sessions, and cloud `/schedule` routines — treat any of those as a bounded dispatch (the second branch below) unless `ScheduleWakeup` is actually observed present in that session's own tool list.

   - **`ScheduleWakeup` present — you are the live interactive session.** Run `watch` as a backgrounded Bash call (`run_in_background: true`) instead of inline. Blocking your own turn on a multi-minute wait would freeze the conversation the user is watching; backgrounding returns control immediately and the harness reliably notifies the live session when a background job it started completes.
   - **`ScheduleWakeup` absent — you are a bounded dispatch** (sub-agent, teammate, or any entity that must return a final result and stop). Run `watch` inline and block on it directly — never background it. A backgrounded process does keep running after its dispatching sub-agent returns and terminates, but nothing ever follows up on it: no notification of any kind reaches the orchestrator or anyone else once the dispatching sub-agent is gone, so its result is silently lost. Blocking costs nothing here — nothing is waiting on this dispatch's live output regardless of how long `watch` takes.
...
- A sub-agent must never background `watch`. Tested directly: a sub-agent dispatched a `sleep 20 && echo done >> file` job with `run_in_background: true`, then returned its own final result immediately (~7s later, well before the sleep finished) without waiting. The job kept running and the file held the expected output after the full 20s — the backgrounded process does survive its dispatching sub-agent's termination. But no notification of any kind arrived when that job finished; every `Agent`/`Task` dispatch this same session generated an automatic completion notification, while the orphaned background job generated none — its result was only found by manually checking the file afterward. This is why the bounded-dispatch branch of step 7 blocks on `watch` directly instead of backgrounding it: backgrounding would run to completion but nothing would ever read the result.
