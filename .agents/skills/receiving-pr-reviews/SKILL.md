...
7. Once all current threads are resolved, re-check for new reviews using the `watch` subcommand — it blocks its own process on an internal polling loop and returns as soon as new activity appears or its timeout elapses, so no separate resumption mechanism (scheduling, wakeups) is needed to check again later:
   ...
   - **`ScheduleWakeup` present — you are the live interactive session.** Run `watch` as a backgrounded Bash call (`run_in_background: true`) instead of inline. Blocking your own turn on a multi-minute wait would freeze the conversation the user is watching; backgrounding returns control immediately and the harness notifies you when the call finishes.
   - **`ScheduleWakeup` absent — you are a bounded dispatch** (sub-agent, teammate, or any entity that must return a final result and stop). Run `watch` inline and block on it directly. Nothing is waiting on your live output, so blocking costs nothing — and whether a Bash background process survives past your own dispatch's completion is unverified, so backgrounding it risks a silently-killed check that never reports anything.
...
- Whether a Bash `run_in_background` process spawned by a sub-agent keeps running after that sub-agent's own dispatch returns its final result is unverified — no documentation source or empirical test available within a single session's own lifecycle settled it. This is why the bounded-dispatch branch of step 7 blocks on `watch` directly instead of backgrounding it.
