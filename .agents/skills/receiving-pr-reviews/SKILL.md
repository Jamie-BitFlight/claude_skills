...
5. Resolve the thread:
   ...
   ```
6. A decision spanning threads (PR sequencing, rebase disposition) goes on the PR itself via `gh pr comment <N> -R <owner>/<repo>` — the same owner/repo this run selected in step 1 (default `Jamie-BitFlight/claude_skills`), not a different repository — before the work it governs.
...
- `watch`'s baseline is its own first fetch, taken when the call starts — a thread that appears in the narrow window between step 5's last `resolve` and that first fetch is folded into the baseline and never shows up in `new_thread_ids`. It still shows up in `state.unresolved_count`, though: read that field on every `watch` result, including a `timed_out: true` one, the same way step 1 requires reading it on `fetch` — do not rely on `new_thread_ids`/`new_reviews_with_body` alone to conclude nothing is outstanding.
