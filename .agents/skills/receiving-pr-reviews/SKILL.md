...
7. Test before running this step: were you dispatched via Task/Agent to complete this skill and must you return a single final result and stop (e.g. you were told "Your ROLE_TYPE is sub-agent")? If yes, you cannot run this step — `ScheduleWakeup` resumes a live turn-taking session, not a terminated Task invocation. State explicitly in your final report that steps 1-6 are complete and that step 7 needs the live session; do not attempt it and do not omit it silently. If no — you are the live interactive session reading this skill directly and `ScheduleWakeup` is available to you — run this step yourself: once all current threads are resolved, check for additional reviews three times at 10-minute intervals via `/loop` (`/schedule`'s Cloud Routines have a 1-hour minimum interval — too coarse for this cadence). A new review restarts this skill from step 1 and cancels the remaining checks. A Codex thumbs-up with no comment, or an explicit "no reviews"/"no changes"/"0 comments" response, is that reviewer's completion signal.
...
- The step 7 test is architectural, not a role label: a bounded Task/Agent dispatch that must return one final result and stop cannot be resumed by `ScheduleWakeup` — that call resumes the live turn-taking session, never a terminated invocation. Whichever entity is in the live conversation loop with no report-and-stop contract is the one that can call `ScheduleWakeup` and actually be resumed by it — that is who runs step 7.
