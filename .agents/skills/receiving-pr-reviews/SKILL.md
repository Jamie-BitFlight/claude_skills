...
7. Step 7 belongs to the orchestrating or interactive session only. A one-shot dispatched worker (subagent, kage-bunshin, or any invocation that returns a single final result and does not resume) MUST NOT attempt this step and MUST NOT omit it silently — state explicitly in the final report that steps 1-6 are complete and that step 7's re-check loop is the orchestrator's responsibility. The orchestrating session, once all current threads are resolved, checks for additional reviews three times at 10-minute intervals via `/loop` (`/schedule`'s Cloud Routines have a 1-hour minimum interval — too coarse for this cadence). A new review restarts this skill from step 1 and cancels the remaining checks. A Codex thumbs-up with no comment, or an explicit "no reviews"/"no changes"/"0 comments" response, is that reviewer's completion signal.
...
- A dispatched worker has no mechanism to pause and resume later — a `ScheduleWakeup`-style delay resumes the calling session's own turn loop, not a subagent invocation that has already returned its final result. This is why step 7 is scoped to the orchestrating session only.
