...
Plugins are expected to be developed cross-harness compatible; "Cross-harness" under Code
Conventions names the supported harnesses.
...
Do not read an ADR for this, and do not link one. An ADR records a deliberation, not the current design; reaching current truth by replaying a chain of decisions is the failure [Architecture Documentation and ADR Lifecycle](rules/adr-lifecycle.md) exists to prevent.
...
`marko` via `uv add marko` if the target project doesn't already depend on it. For choosing a
data format — the format closest to the consumer, never prose parsed for your own data — read
[rules/data-format-selection.md](rules/data-format-selection.md).
...
[rules/skill-content-optimization.md](rules/skill-content-optimization.md). Before writing into one
agent a process another agent already carries (output format, status messaging,
definition-of-done, environment resolution), read
[rules/shared-process-extraction.md](rules/shared-process-extraction.md): one skill holds it,
each agent loads it.
