...
9. **ty language server false positives on PEP 723 scripts**: ty (0.0.75-0.0.80) type-checks any `.py` file with a `# /// script ... # ///` block as an isolated single-file project and never consults `[tool.ty.environment]` for it — `extra-paths`, `root`, and `python` are all silently ignored (upstream, open: https://github.com/astral-sh/ty/issues/691). If `uv run ty check <path>` passes clean but the live LSP diagnostic shows `unresolved-import` on a PEP 723 script's own declared dependency, that is this bug, not a missing `extra-paths` entry — see `rules/python-development.md`'s PEP 723 language-server section for the fix (`.claude/settings.json`'s `env.VIRTUAL_ENV`) and `tests/test_ty_pep723_environment.py` for the regression coverage.
