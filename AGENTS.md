...
9. **ty PEP 723 script resolution — experimental `TY_UV`/`useUv`, not `VIRTUAL_ENV`**: ty (through at least 0.0.80) type-checks any `.py` file with a `# /// script ... # ///` block as an isolated single-file project and, by default, never consults `[tool.ty.environment]` for it — `extra-paths`, `root`, and `python` are all silently ignored (upstream, open: https://github.com/astral-sh/ty/issues/691). Astral shipped an experimental, opt-in fix on 2026-08-28 (requires uv ≥0.12.3): `TY_UV=scripts` for the CLI, `ty.experimental.useUv` (VS Code, checked in at `.vscode/settings.json`) or the equivalent `initialization_options.experimental.useUv` for other language-server clients — ty then shells out to `uv` to sync the script's own inline dependencies, in both the CLI and the LSP. This repo's CLI usage (`uv run ty check`, prek, CI) already resolves PEP 723 scripts correctly today because their dependencies are mirrored into the root `[dependency-groups] dev` group (see `rules/python-development.md`); the opt-in above fixes the language-server-only false positive. No `.claude/settings.json` edit is needed or used — see `rules/python-development.md`'s PEP 723 language-server section for the fix and `tests/test_ty_pep723_environment.py` for the regression coverage.
