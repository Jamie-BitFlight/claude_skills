...
Call `uv run --script "${CLAUDE_PLUGIN_ROOT}/sam_schema/cli.py" backlog view --selector "<item_ref/>"` (accepts URLs, `#N`, bare numbers, and title substrings).
...
4. Call `uv run --script "${CLAUDE_PLUGIN_ROOT}/sam_schema/cli.py" backlog close`:
...
2. If `**Plan**:` is present, check the GitHub Issue labels for `status:verified`:
   - Call `uv run --script "${CLAUDE_PLUGIN_ROOT}/sam_schema/cli.py" backlog view --selector "{title}"` and inspect the `labels` list in the returned dict.
...
2. Extract the plan address from the plan path (e.g., `plan/Pe9f0a1b2-backlog-lifecycle-process-gaps.yaml` → `Pe9f0a1b2`). Call:
   ...
   ```bash
   uv run --script "${CLAUDE_PLUGIN_ROOT}/sam_schema/cli.py" plan status --plan-address "{address}"
   ```
...
4. Extract acceptance criteria from the `backlog_view` response. Call `uv run --script "${CLAUDE_PLUGIN_ROOT}/sam_schema/cli.py" backlog view --selector "{title}"`, then read `response["sections"]["Acceptance Criteria"]`. The field format is a bullet list:
...
8. If the item has a linked GitHub Issue (`#N`), check whether an open PR already references it:
   ...
   - **Open PR found**: The PR body contains `Fixes #N` — the issue will auto-close on merge. Update only the backlog item status (do NOT close the GitHub Issue):
     ...
     Call `uv run --script "${CLAUDE_PLUGIN_ROOT}/sam_schema/cli.py" backlog update --selector "{title}" --status "in-progress"`.
...
11. Call `uv run --script "${CLAUDE_PLUGIN_ROOT}/sam_schema/cli.py" backlog resolve`:
...
13. Before emitting Handoff E, determine whether all milestone issues are resolved:
    ...
    b. If a milestone is present, call:
       ...
       ```bash
       uv run --script "${CLAUDE_PLUGIN_ROOT}/sam_schema/cli.py" backlog list
       ```
