...
1. Read the plan via `uv run --script "${CLAUDE_PLUGIN_ROOT}/sam_schema/cli.py" plan read --address {plan_address}`.
...
```bash
uv run --script "${CLAUDE_PLUGIN_ROOT}/sam_schema/cli.py" backlog groom --selector "{title}" --section "Plan Drift" --content "{findings}"
```
...
1. Call `uv run --script "${CLAUDE_PLUGIN_ROOT}/sam_schema/cli.py" backlog view --selector "{title}"` (the CLI has no `summary` parameter — it always returns full content).
...
```bash
uv run --script "${CLAUDE_PLUGIN_ROOT}/sam_schema/cli.py" backlog groom --selector "{title}" --section "Grooming Drift" --content "{findings}"
```
