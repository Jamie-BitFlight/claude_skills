...
Verify <item_ref/> exists via `uv run "${CLAUDE_PLUGIN_ROOT}/sam_schema/cli.py" backlog view --selector "{item_ref}"`
...
```bash
uv run "${CLAUDE_PLUGIN_ROOT}/sam_schema/cli.py" backlog merged-prs --search "{title keywords}"
```

If commits or merged PRs reference this item:

```bash
uv run "${CLAUDE_PLUGIN_ROOT}/sam_schema/cli.py" backlog resolve --selector "{item_ref}" --summary "Completed via PR #{pr} / commit {sha}"
```

If the active backend supports comments, also record evidence:

```bash
uv run "${CLAUDE_PLUGIN_ROOT}/sam_schema/cli.py" backlog comment-issue --issue-number {issue_number} --body "Completed via {evidence}"
```
...
```bash
uv run "${CLAUDE_PLUGIN_ROOT}/sam_schema/cli.py" backlog merged-prs --search "{item_ref}"
```
