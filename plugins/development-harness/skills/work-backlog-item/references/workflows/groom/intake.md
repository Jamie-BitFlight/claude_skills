...
```bash
backlog merged-prs --search "{title keywords}"
```

If commits or merged PRs reference this item:

```bash
backlog resolve --selector "{item_ref}" --summary "Completed via PR #{pr} / commit {sha}"
```

If the active backend supports comments, also record evidence:

```bash
backlog comment-issue --issue-number {issue_number} --body "Completed via {evidence}"
```
...
```bash
backlog merged-prs --search "{item_ref}"
```
