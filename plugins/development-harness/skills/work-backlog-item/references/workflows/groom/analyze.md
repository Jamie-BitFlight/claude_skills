...
```bash
uv run "${CLAUDE_PLUGIN_ROOT}/sam_schema/cli.py" artifact list --item-id {issue_number} --artifact-type feature-context
```

2. If `count > 0`: load the artifact and pass it to swarm agents as prior context.

```bash
uv run "${CLAUDE_PLUGIN_ROOT}/sam_schema/cli.py" artifact read --item-id {issue_number} --artifact-type feature-context
```
...
```bash
uv run "${CLAUDE_PLUGIN_ROOT}/sam_schema/cli.py" backlog groom --selector "{item_ref}" --section "RT-ICA" --content "{snapshot}"
```
