...
```bash
artifact list --item-id {issue_number} --artifact-type feature-context
```

2. If `count > 0`: load the artifact and pass it to swarm agents as prior context.

```bash
artifact read --item-id {issue_number} --artifact-type feature-context
```
...
logical identifier (see [Artifact Conventions](../../../../dh-meta-docs/references/artifact-conventions.md#logical-identifier-conventions)).
