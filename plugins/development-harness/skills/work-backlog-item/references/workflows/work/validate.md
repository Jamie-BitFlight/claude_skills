...
**Two-part step:** (a) Always run `uv run --script "${CLAUDE_PLUGIN_ROOT}/sam_schema/cli.py" backlog update --selector "{title}" --status "in-progress"` for the current item. (b) Run `milestone start` only on explicit user intent to start the whole milestone — it bulk-transitions all open milestone issues, not just the current one.
...
`uv run --script "${CLAUDE_PLUGIN_ROOT}/sam_schema/cli.py" artifact list --item-id {N} --artifact-type feature-context`.
