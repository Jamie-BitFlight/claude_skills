...
- For every complete annotation, run
  `rg --hidden --glob '!**/.git' --glob '!**/.git/**' -F -l -- "$pattern"` and capture its output
  and exit status before counting lines. This includes tracked hidden paths while excluding Git
  internals and worktree pointer files at any depth. Exit status 0 means matches.
  Exit status 1 means zero matches. An
