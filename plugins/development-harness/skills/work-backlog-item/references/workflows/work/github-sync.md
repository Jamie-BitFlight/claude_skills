...
Call the CLI: `uv run --script "${CLAUDE_PLUGIN_ROOT}/sam_schema/cli.py" backlog update --selector "{title}"`.
...
Call the CLI: `uv run --script "${CLAUDE_PLUGIN_ROOT}/sam_schema/cli.py" backlog update --selector "{title}" --status "in-progress"`. Check the JSON output for an `error` key.
