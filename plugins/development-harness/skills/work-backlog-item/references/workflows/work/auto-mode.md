...
|---|---|
| No title given (<item_ref/> is empty) | Call the CLI: `uv run --script "${CLAUDE_PLUGIN_ROOT}/sam_schema/cli.py" backlog list` and scan P0 then P1 sections in the JSON output for the first open item. Log `[AUTO] No title — auto-selected: {title}` and proceed. If none found, log `[AUTO] STOP — no open P0/P1 items found` and stop. |
