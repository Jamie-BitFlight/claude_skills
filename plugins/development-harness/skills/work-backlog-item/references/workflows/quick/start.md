...
3. Find the item via the CLI: `uv run "${CLAUDE_PLUGIN_ROOT}/sam_schema/cli.py" backlog view --selector "{title or #N}"`. If not found (JSON output contains an `error` key), create a minimal item:
...
6. Call the CLI to record the plan reference: `uv run "${CLAUDE_PLUGIN_ROOT}/sam_schema/cli.py" backlog update --selector "{title}" --plan "{plan_id from step 5}"`.
