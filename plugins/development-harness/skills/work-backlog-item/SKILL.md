...
For each placeholder in the form <key/> whose key exists in `<input/>`, substitute that input value.
`<sam_cli/>` and `<dh_scripts/>` are command tokens, not input placeholders; resolve them by loading
`dh:dh-cli-usage` before running commands.

The `references/workflows/*.md` files loaded by this skill are plain files, not substituted — they show bare SAM CLI subcommands and args only (e.g. `backlog view --selector "..."`), never the invocation prefix. Prepend the resolved `<sam_cli/>` command to every one of them.
...
**MCP server availability**: Both `plugin:dh:backlog` and `plugin:dh:sam` initialize in ~1–2 seconds after a session restart. Claude Code handles connection waiting automatically. If a tool is unavailable, see the troubleshooting steps at dh:dh-cli-usage MCP connection check — its commands use the `<sam_cli/>` and `<dh_scripts/>` values above.
