...
Call `uv run --script "${CLAUDE_PLUGIN_ROOT}/sam_schema/cli.py" backlog list --status "resolved"` (the CLI's `backlog list` has no `--search` full-text flag, but `--status` used here is fully supported), filter by keywords related to this defect class, count matches, and write the measurement section:
