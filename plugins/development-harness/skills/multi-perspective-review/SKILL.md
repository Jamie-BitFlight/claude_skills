...
the SAM task operations `sam_plan`, `sam_task`, and `sam_active_task`, so none of them can claim or
close a task; their behavior reaches the work as a loaded profile instead.
...
- Dispatch uses `dh:task-worker` because the perspective reviewer agents cannot claim or close a SAM task. Specialist behavior is selected through the task profile.
