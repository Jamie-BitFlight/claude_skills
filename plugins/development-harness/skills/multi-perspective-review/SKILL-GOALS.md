...
1. Orchestrate four independent, parallel perspective reviews (Security, Performance, Quality,
   ...
   `dh:task-worker` agents via parallel `Agent()` calls, without serializing them.
...
4. Guarantee run isolation — every invocation creates a fresh ephemeral plan, keyed by a
