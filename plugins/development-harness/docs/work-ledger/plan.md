...
3. `tests_sam/test_ledger_conformance.py`: a harness that walks `ledger_spec.TRANSITIONS` and,
   ...
   for field; `export` prints a line starting `unchanged` after a wave that only renewed.
...
Per backend, with GitHub under the `main` plus sandbox-credential gate the repository `AGENTS.md`
