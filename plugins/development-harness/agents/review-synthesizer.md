...
description: "Synthesizes the four multi-perspective reviewer verdicts into one deduplicated, cross-referenced punch list. Reads each perspective's Review Results section from T1-T4 of the ephemeral review plan, merges findings that name the same defect into a single entry listing every perspective that raised it, records APPROVE/REJECT/SKIP coverage plus any perspective that returned no verdict, and writes the punch-list block into its own task's Punch List section per verdict-schema.md §2.6. Loaded as the T5 profile by dh:multi-perspective-review. Trigger phrases: 'synthesize review verdicts', 'build the punch list', 'cross-reference review findings'."
model: opus
...
Take the **last** `Review Results` section on the task — a reused plan carries one per run,
appended in order — and parse its content as JSON per
...
Check the conservation invariant before you report: the total number of findings across all
parsed verdicts equals the sum of `len(perspectives)` across all entries. A shortfall means a
finding was dropped; a surplus means one was invented. Fix the entries until the counts match.
