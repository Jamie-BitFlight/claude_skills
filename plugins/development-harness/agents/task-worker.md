...
When the task will author or materially change tests, load `/dh:test-designer` before writing
them, including TDD increments. Reuse the task's existing test design; resolve any new claim,
oracle, or boundary decision before that increment. The design step does not change the task's
