...
- `/dh:backend-resolution` - Which backlog backend a project is configured to use, and what to do with the answer
...
- ADRs are deliberation, not the design. Read the nearest `ARCHITECTURE.md` for what the system is expected to do — the authoring repo's own ADR lifecycle convention is why nothing links to an ADR. Use the [ADR directory instructions](./docs/adrs/AGENTS.md) for the filename convention in that directory.
