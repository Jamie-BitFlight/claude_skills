...
async def call_mcp_tool(
    mcp: FastMCP, tool_name: str, params: dict | None = None, *, timeout_seconds: float | None = None
) -> dict:
    ...
    Args:
        ...
        timeout_seconds: Optional whole-tool deadline. Initialization remains
            bounded at 30 seconds independently. Named
    ...
    async with Client(mcp, timeout=None, init_timeout=30.0) as client:
        if timeout_seconds is None:
            result = await client.call_tool(tool_name, params or {})
        else:
            result = await client.call_tool(tool_name, params or {}, timeout=timeout_seconds)
