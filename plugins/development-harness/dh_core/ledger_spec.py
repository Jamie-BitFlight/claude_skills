...
COLUMNS: list[Column] = [
    ...
    *_cols(
        ...
        Provenance.EVENT,
        [
            "merge.candidate-submitted",
            "merge.candidate-admitted",
            "merge.candidate-enqueued",
            "merge.candidate-superseded",
            "merge.finished",
            "merge.blocked",
            "merge.reconciled",
        ...
        maker_task="text",
        maker_attempt="int",
        maker_evidence_digest="text",
        submitted_seq="int",
        supersedes_candidate_number="int|null",
        superseded_seq="int|null",
        checker_issue="int|null",
        checker_task="text|null",
        checker_attempt="int|null",
        checker_evidence_digest="text|null",
        policy_snapshot_digest="text|null",
        admitted_seq="int|null",
        enqueued_seq="int|null",
        outcome="text|null",
        result_sha="text|null",
        finished_seq="int|null",
    ),
    *_cols(
        "merge_claims",
        Provenance.EVENT,
        [
            "merge.claimed",
            "merge.claim-bound",
            "merge.claim-prepared",
            "merge.claim-renewed",
            "merge.claim-recovered",
            "merge.finished",
            "merge.blocked",
        ...
        integrator_task="text",
        integrator_attempt="int",
        phase="text",
        expires="text",
        expected_target_sha="text|null",
        expected_candidate_sha="text|null",
        policy_snapshot_digest="text|null",
        result_sha="text|null",
...
EVENTS: list[EventKind] = [
    ...
    EventKind(
        ...
        payload=[
            ...
            "supersedes_candidate_number",
        ],
        written_by=["merge-submit"],
    ),
    EventKind(
        kind="merge.candidate-superseded",
        payload=["generation", "candidate_number", "replacement_candidate_number"],
        written_by=["merge-submit"],
    ),
    EventKind(
        ...
        written_by=["merge-admit"],
    ),
    EventKind(kind="merge.candidate-enqueued", payload=["generation", "candidate_number"], written_by=["merge-admit"]),
    EventKind(
        kind="merge.claimed",
        payload=[
            "claim_number",
    ...
    EventKind(kind="merge.claim-renewed", payload=["claim_number", "phase", "expires"], written_by=["merge"]),
    EventKind(
        kind="merge.claim-recovered", payload=["claim_number", "conclusion"], written_by=["merge", "merge-reconcile"]
    ),
    EventKind(
        kind="merge.finished",
        payload=[
    ...
    EventKind(
        kind="merge.blocked",
        payload=["generation", "candidate_number", "claim_number", "conclusion"],
        written_by=["merge"],
    ),
    EventKind(
        kind="merge.reconciliation-required",
        payload=["claim_number", "phase", "result_sha", "prepared_identity_digest", "policy_snapshot_digest"],
        written_by=["merge", "merge-reconcile"],
    ),
    EventKind(
        kind="merge.reconciled",
        payload=[
            "generation",
            "candidate_number",
            "claim_number",
        ...
        written_by=["merge-reconcile"],
    ),
    EventKind(
        kind="merge.reconciliation-resolved",
        payload=["generation", "candidate_number", "claim_number", "conclusion", "policy_snapshot_digest"],
        written_by=["merge-reconcile"],
    ),
...
TRANSITIONS: list[Transition] = [
    ...
    *[
    ...
    Transition(
        command="merge-submit",
        from_status=ANY,
        checks=[],
        effects=[Effect(column="merge_candidates", value="immutable numbered candidate")],
        events=["merge.candidate-submitted"],
        conditional_events=["merge.candidate-superseded"],
    ),
    Transition(
        command="merge-admit",
        from_status=ANY,
        checks=[],
        effects=[Effect(column="merge_candidates", value="checker admission and queue sequence")],
        events=["merge.candidate-admitted", "merge.candidate-enqueued"],
    ),
    Transition(
        command="merge",
        from_status=ANY,
        checks=[],
        effects=[Effect(column="merge_claims", value="three-phase claim and exact outcome")],
        events=["merge.claimed", "merge.claim-bound", "merge.claim-prepared"],
        conditional_events=[
            "merge.claim-renewed",
            "merge.claim-recovered",
            "merge.finished",
    ...
    Transition(
        command="merge-reconcile",
        from_status=ANY,
        checks=[],
        effects=[Effect(column="merge_claims", value="observation-only resolution")],
        events=[],
        conditional_events=[
            "merge.claim-recovered",
            "merge.reconciliation-required",
