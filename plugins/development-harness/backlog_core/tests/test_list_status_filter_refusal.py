...
class TestDerivedStatusWithoutALiveAnswer:
    ...
    def test_a_missing_key_in_a_live_map_is_unknown(self) -> None:
        """A successful query that omitted the issue did not establish its status."""
        assert operations._item_derived_status(_item("#42"), {}, status_live=True) is None
    ...
    def test_a_string_issue_reference_keeps_its_needs_grooming_default(self) -> None:
        """Backend-owned string identifiers use their cached status rather than the numeric live map."""
...
class TestStatusFilterWithALiveAnswer:
    ...
    issue that needs grooming under a live answer, whether its fetched map entry
    has no status label or explicitly carries ``status:needs-grooming``."""
    ...
    def test_a_missing_status_label_still_matches_the_bare_filter(self, mocker: MockerFixture) -> None:
        ...
        mocker.patch.object(operations, "batch_fetch_statuses", return_value={42: IssueStatus(state="OPEN")})

        result = operations.list_items(status="needs-grooming", output=Output())

        assert result["count"] == 1

    def test_a_closed_fetched_issue_without_a_status_label_is_not_fabricated_as_needs_grooming(
        self, mocker: MockerFixture
    ) -> None:
        """A cached closed item included in the listing keeps the fetched blank status."""
        closed = _item("#42", title="Closed", status="closed")
        _patch_backend(mocker, [closed])
        mocker.patch.object(operations, "batch_fetch_statuses", return_value={42: IssueStatus(state="CLOSED")})

        unfiltered = operations.list_items(include_closed=True, output=Output())
        filtered = operations.list_items(include_closed=True, status="needs-grooming", output=Output())

        assert unfiltered["count"] == 1
        assert _statuses(unfiltered) == [""]
        assert unfiltered["status_source"] == "live"
