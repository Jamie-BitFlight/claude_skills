...
# ---------------------------------------------------------------------------
# Generator/validator agreement property (#2969 Finding 2)
# ---------------------------------------------------------------------------

# Root-level ordinals are plain digit strings produced elsewhere in the mapper
# (top-level section index), not by either generator under test.
...
def _heading_path(draw: st.DrawFn) -> str:
    ...
    Mirrors the recursion in ordinal_mapper.py: each level's ``sub_ordinal``
    becomes the ``parent_ordinal`` for the next level's
...
class TestOrdinalGeneratorValidatorAgreement:
    ...
    ``_entry_ordinal_for_sub_heading()`` and ``_entry_ordinal_for_code()`` build
    ordinal-path strings by plain f-string composition, while
    ``disclosure_handler._ORDINAL_PATTERN`` independently hand-writes a regex
    describing the same shape. Nothing else enforces that the two agree — a change
    to either side could silently desynchronize them, the same risk class #2956
    fixed for section-key derivation (see ``TestSectionRoundTripProperty`` in
    ``test_github_sync.py``, whose style this follows).
    """
