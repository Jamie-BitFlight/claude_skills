...
from backlog_core.disclosure_types import BoundedResponse, MapResponse, NavigateResponse, OrdinalNotFoundError
...
_skip_without_real_enc = pytest.mark.skipif(
    not _ENCODING_AVAILABLE,
    reason="cl100k_base encoding unavailable (offline environment).",
)
...
def _find_rt_ica_ordinal(normalized: list[NormalizedSection]) -> str:
    ...
    if not rt_ica_entries:
        ...
        raise ValueError(
            f"No OrdinalEntry with 'RT-ICA' in title found in map. "
...
class TestMapTotalEstTokensLevelOneOnly:
    ...
    def test_total_est_tokens_equals_level1_sum_only(
        self,
        multi_entry_view_result: ViewItemResult,
        ...
        assert isinstance(result, MapResponse), (
            f"Expected MapResponse; got {type(result).__name__}."
        )
    ...
    def test_map_response_has_non_empty_map_text(
        ...
        assert result.map_text, (
            "map_text must not be empty for a document with sections."
        )
...
class TestNavigateMode:
    ...
    def test_navigate_returns_navigate_response(
        ...
        assert isinstance(result, NavigateResponse), (
            f"Expected NavigateResponse; got {type(result).__name__}."
        )
    ...
    def test_navigate_content_is_non_empty(
        ...
        assert result.content, (
            "NavigateResponse.content must not be empty for ordinal '0'."
        )
    ...
    def test_navigate_total_tokens_is_positive(
        ...
        assert result.total_tokens > 0, (
            "NavigateResponse.total_tokens must be positive for non-empty content."
        )
...
class TestExtractModeRTICA:
    ...
    def test_extract_returns_bounded_response(
        ...
        assert isinstance(result, BoundedResponse), (
            f"Expected BoundedResponse; got {type(result).__name__}."
        )
    ...
    def test_extract_returned_tokens_within_head_bound(
        ...
        assert result.returned_tokens <= 100, (
            f"returned_tokens must be ≤ head=100; got {result.returned_tokens}."
        )
    ...
    def test_extract_next_call_is_on_bounded_response_not_bounded_content(
        ...
        assert hasattr(result, "next_call"), (
            "BoundedResponse.next_call field must exist on the returned object."
        )
...
class TestUngatedViewItemPath:
    ...
    @_skip_without_2515
    @_skip_without_real_enc
    def test_handle_calls_view_item_exactly_once(
        self,
        view_result_2515: ViewItemResult,
        mocker: MockerFixture,
    ) -> None:
        """handle() calls ``operations.view_item()`` exactly once (un-gated path)."""
        view_item_spy = mocker.patch(
            "backlog_core.operations.view_item",
            return_value=view_result_2515,
        )

        BacklogViewDisclosureHandler().handle(
            "#2515", DisclosureRequestParser().parse(map=True)
        )
    ...
    @_skip_without_2515
    @_skip_without_real_enc
    def test_handle_passes_selector_to_view_item(
        self,
        view_result_2515: ViewItemResult,
        mocker: MockerFixture,
    ) -> None:
    ...
    def test_overbudget_item_produces_non_empty_map_text(
        ...
        assert result.total_sections > 0, (
            f"total_sections must be > 0 for #2515; got {result.total_sections}."
        )
