...
class _ProxyAwareAdapter(requests.adapters.HTTPAdapter):
    ...
    def init_poolmanager(
        ...
        """
        pool_kwargs["ssl_context"] = build_ssl_context(self._ca_bundle)
    ...
    def proxy_manager_for(self, proxy: str, **proxy_kwargs: object) -> PoolManager:
        ...
        """
        proxy_kwargs["ssl_context"] = build_ssl_context(self._ca_bundle)
        return super().proxy_manager_for(proxy, **proxy_kwargs)


def make_connection_class(ca_bundle: str) -> type[HTTPSRequestsConnectionClass]:
...
def install_proxy_tls_support(*, force: bool = False) -> bool:
    ...
    """
    with InstallState.lock:
        if InstallState.installed and not force:
            return True
        ca_bundle = resolve_ca_bundle()
        if ca_bundle is None:
            return False
        Requester.injectConnectionClasses(HTTPRequestsConnectionClass, make_connection_class(ca_bundle))
