...
**Portability check**: every path, link, and cross-plugin reference in the refactored agent's runtime text passes the three-part test in `${CLAUDE_PLUGIN_ROOT}/docs/runtime-environment.md` — universally present, bundled with the artifact, or inlined. Report each failure with file:line.
