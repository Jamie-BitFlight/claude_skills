...
**Always in scope, whatever else changes**: every path, link, and cross-plugin reference in runtime text passes the three-part test — the path is present in every environment, bundled with the artifact, or inlined in the text. Report each failure with file:line. Rewriting a sentence around a path that only resolves in the authoring repo leaves the defect intact and better phrased.
...
**Ecosystem-owned key pass-through:** Treat `mcp:` as an OpenCode ecosystem-owned field — copy it verbatim into the output. Do not rewrite, normalize, reorder, or remove it. It is not subject to Claude Code schema validation.
