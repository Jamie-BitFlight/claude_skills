...
Reliability includes resolving elsewhere: every path, link, and cross-plugin reference in runtime text passes the three-part test — the path is present in every environment, bundled with the artifact, or inlined in the text. Report each failure with file:line.
