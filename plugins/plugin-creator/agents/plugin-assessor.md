...
VERIFY every path, link, and cross-plugin reference in runtime text passes the three-part test — the path is present in every environment, bundled and reached by a relative path that stays inside the plugin, or inlined — and report each failure with file:line.
