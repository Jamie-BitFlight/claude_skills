...
description: Validate and pre-fetch SOURCE URLs from a skill-sync change plan. Use between skill-sync synthesis and write execution.
model: sonnet
...
1. Search for the project's GitHub repo:
   ...
   ```
2. Confirm the repository URL and that its documentation boundary matches the failed source domain.
...
When every URL validates and no downgrade is needed, include `UNVERIFIABLE downgrades: 0 (none)` in the `STATUS: DONE` output.
