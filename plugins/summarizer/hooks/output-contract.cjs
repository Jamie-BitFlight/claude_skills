...
const METADATA = [
  'source_type', 'source_path', 'summarized_at', 'method', 'word_count_source',
  'word_count_summary', 'confidence', 'confidence_notes',
];
...
function validateJson(text) {
  let value;
  try { value = JSON.parse(text); } catch { return ['Output must be raw, parseable JSON.']; }
  ...
  if (!object(value.metadata)) errors.push('metadata must be an object.');
    ...
    if (!['high', 'medium', 'low'].includes(value.metadata.confidence)) errors.push('Invalid confidence.');
    if (typeof value.metadata.confidence_notes !== 'string' || !value.metadata.confidence_notes.trim()) {
      errors.push('confidence_notes must explain the assessment.');
    }
  }
  if (typeof value.summary !== 'string' || !value.summary.trim()) errors.push('summary must be nonempty.');
  ...
  for (const finding of Array.isArray(value.findings) ? value.findings : []) {
    if (!object(finding) || typeof finding.item !== 'string' || !finding.item.trim()
      || typeof finding.source_ref !== 'string' || !finding.source_ref.trim()) {
  ...
  for (const source of Array.isArray(value.sources) ? value.sources : []) {
    if (!object(source) || typeof source.path !== 'string' || !source.path.trim()
      || typeof source.accessed !== 'string' || !source.accessed.trim()) {
...
function validate(text, format) {
  ...
  if (format === 'structured') {
    ...
    if (!fm) errors.push('Missing YAML frontmatter.');
      ...
      for (const key of METADATA) {
        if (!new RegExp(`^${key}\\s*:`, 'm').test(fm[1])) errors.push(`Missing frontmatter: ${key}`);
      }
    }
    let previous = -1;
    for (const section of SECTIONS) {
      const index = found.indexOf(section);
      if (index <= previous || index < 0) errors.push(`Missing or out-of-order section: ${section}`);
      previous = index;
    }
  } else {
    ...
    if (format === 'tldr') {
      ...
      if (!required.every((name) => header.includes(name))) errors.push('Missing table columns.');
        ...
        for (const row of rows.slice(2)) {
          ...
          statuses.add(status);
          if (row.length !== header.length || !['Found', 'Not Found', 'Uncertain'].includes(status)) {
        ...
        for (const status of ['Not Found', 'Uncertain']) {
          if (!statuses.has(status)) errors.push(`Missing ${status} row (use None identified when empty).`);
        }
      }
      if (!found.includes('Summary')) errors.push('Missing Summary section.');
    } else {
      const required = format === 'bullets' ? ['Key Findings', 'Not Found', 'Uncertain']
        : ['Outline', 'Not Found', 'Uncertain'];
      for (const section of required) if (!found.includes(section)) errors.push(`Missing section: ${section}`);
    }
  }
  return errors;
}
...
if (require.main === module) {
  try {
    const { values } = parseArgs({ options: { format: { type: 'string' }, input: { type: 'string' } } });
