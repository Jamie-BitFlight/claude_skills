...
function requestFrom(records) {
  ...
  if (output && !path.isAbsolute(output))
    throw new Error('SUMMARIZER_OUTPUT must be an absolute caller-assigned path.');
  return { format: formats[0][1], output };
}

function decide(data) {
  if (data?.hook_event_name !== 'SubagentStop' || !AGENTS.test(data.agent_type || ''))
    return { code: 0 };
  try {
    if (!data.agent_transcript_path)
      throw new Error('No transcript available for caller controls.');
    const records = fs
      .readFileSync(data.agent_transcript_path, 'utf8')
      .split('\n')
      .filter((line) => line.trim())
      .map((line) => JSON.parse(line));
    const request = requestFrom(records);
    const last = records
      .filter((r) => r.type === 'assistant' && r.message?.role === 'assistant')
      .at(-1);
    const message =
    ...
    if (/^STATUS: (BLOCKED|PARTIAL)\r?\n/.test(message)) {
      return {
        code: 0,
        notice:
          'NOT_VALIDATED: caller received a non-success status; preserve its reasons and remaining scope.',
      };
    }
    let text = message;
    if (request.output) {
      if (!message.includes(request.output))
        throw new Error('Final response does not reference the assigned summary artifact.');
    ...
    if (data.stop_hook_active === true) {
      return {
        code: 0,
        notice: `NOT_VALIDATED after correction attempt: ${reason}. Caller must validate before accepting DONE.`,
      };
    }
    return {
      code: 2,
      reason: `${reason}. Correct once; if blocked, return STATUS: BLOCKED with the reason.`,
    };
  } catch (error) {
    return {
      code: 0,
      notice: `NOT_VALIDATED: ${error.message}. Run explicit validation before accepting the result.`,
    };
  }
}
...
if (require.main === module) {
  let result;
  try {
    result = decide(JSON.parse(fs.readFileSync(0, 'utf8')));
  } catch (error) {
    result = { code: 0, notice: `NOT_VALIDATED: ${error.message}` };
  }
  if (result.reason) process.stderr.write(`[summarizer-hook] ${result.reason}\n`);
  if (result.notice)
    process.stdout.write(
      `${JSON.stringify({ systemMessage: `[summarizer-hook] ${result.notice}` })}\n`,
    );
