...
const metadata = {
  ...
  summarized_at: '2026-09-25T00:00:00Z',
  method: 'extractive',
  word_count_source: 12,
...
const outputs = {
  ...
  json: JSON.stringify({
    metadata,
    summary: '7 of 10 found; 3 requests timed out.',
    findings: [{ item: '7 of 10 found', source_ref: 'line 1' }],
    not_found: [],
    uncertain: [],
...
test('explicit source negatives and unquantified source prose are not regex failures', () => {
  assert.deepEqual(
    validate(outputs.structured.replace('feature X', 'most features'), 'structured'),
    [],
  );
});
...
test('unavailable metadata is visible and is not success evidence', () => {
  const result = decide({
    ...
    agent_type: 'summarizer:file-summarizer',
  });
...
test('unrelated agents do not access transcripts', () => {
  assert.deepEqual(decide({ hook_event_name: 'SubagentStop', agent_type: 'general-purpose' }), {
    code: 0,
  });
});
...
test('standalone validation uses explicit caller format and nonzero failures', (t) => {
  ...
  const valid = spawnSync(process.execPath, [command, '--format', 'json', '--input', file], {
    encoding: 'utf8',
  });
  assert.equal(valid.status, 0);
  assert.equal(JSON.parse(valid.stdout).status, 'STRUCTURE_VALID');
  const invalid = spawnSync(process.execPath, [command, '--format', 'tldr', '--input', file], {
    encoding: 'utf8',
  });
