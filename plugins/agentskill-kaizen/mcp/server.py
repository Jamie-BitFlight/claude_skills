...
async def discover_process_model(
    ...
    await context.info("Building process model from sequences...")
    process_model = await asyncio.to_thread(_build_process_model, resolved)
...
async def check_conformance(
    ...
    *,
    ...
    Builds a reference transition model, then checks target sessions for
    unseen starts, transitions, and endings.
    ...
    await context.info("Building reference transition model...")
    reference_model = await asyncio.to_thread(_build_process_model, resolved_ref)
    if reference_model.event_count == 0:
        raise ToolError("Reference process model is empty after building from sequences")

    await context.info("Checking target sessions against reference model...")
    return await asyncio.to_thread(check_sequence_conformance, resolved_target, reference_model)
...
async def cluster_sessions(
    ...
    *,
    ...
    Encodes each session's tool sequence as a bag-of-tools vector, then
    groups sessions using deterministic cosine-similarity clustering.
    ...
    effective_clusters = min(n_clusters, len(resolved))
    if effective_clusters < 1:
        raise ToolError("Need at least 1 session to cluster")

    await context.info(f"Clustering {len(resolved)} sessions into {effective_clusters} clusters...")
