...
- `uv run --quiet --script`: Tells uv to:
  - Read PEP 723 metadata from the script
  - Install declared dependencies automatically
  - Execute the script in an isolated ephemeral environment
- For a portable standalone script, omit `--active`: it runs against the caller's activated virtual
  environment and installs the script's dependencies there. Use `--active` only when the script is
  intentionally coupled to that pre-activated environment.
...
The inline block governs the script's PyPI dependencies, not its file count. A script that outgrows
a single file imports its own modules; how those imports resolve depends on where the modules sit.

A sibling module in the script's own directory imports by name with no setup — `uv run` puts the
script's directory first on `sys.path` whatever the working directory:
...
# root = ["."]
# ///
"""Entry script; helpers.py sits beside it."""

from helpers import load_config
```

A script that lives *inside* a package and imports that package by name needs the package's parent
...
# root = [".", ".."]
# ///
"""Entry script living at mypkg/cli.py."""
...
from mypkg import commands, models
```

Only the entry script carries the shebang and the `# /// script` block; the modules it imports are
plain `.py` files with neither.

The `[tool.ty.environment] root` entry inside the block is what lets `ty` resolve those imports.
Without it `ty` reports `unresolved-import` for modules that exist on disk. When editor diagnostics
and `uv run ty check <path>` disagree about such a script, the CLI is right. Load
`python-engineering:ty` before changing either setting — it holds the full rule, including when
`root` applies rather than `extra-paths` and the `TY_UV` environment fix.
...
### Using /python-engineering:shebangpython Command

The `/python-engineering:shebangpython` command validates PEP 723 compliance:
...
See: `../../shebangpython/SKILL.md` or activate with `/python-engineering:shebangpython`
...
4. **Shebang Requirements**
   ...
   - With PEP 723: `#!/usr/bin/env -S uv run --quiet --script`
...
- **Shebang Validation**: `../../shebangpython/SKILL.md` or `/python-engineering:shebangpython`
