...
class MyApp(App):
    ...
    async def long_task(self) -> None:
        ...
        for i in range(100):
            ...
            # In an async worker, direct widget updates are safe.
            # call_from_thread() is only required in thread workers (thread=True).
...
class MyApp(App):
    def on_mount(self) -> None:
        # Pass the callable itself (no parentheses) for thread workers with sync functions.
        # Calling self.fetch_data() would pass the return value (None), not the function.
...
class MyApp(App):
    def on_mount(self) -> None:
        # Pass name at creation time — Worker.name is not a writable attribute.
        worker = self.run_worker(self.risky_task(), name="data_processor")
...
# Decorate the calling method with @work to ensure it runs in a worker context.
# SOURCE: https://textual.textualize.io/guide/screens/ — "Waiting for screens"
...
NOTE: The official Textual testing guide uses bare `async def` functions without
`@pytest.mark.asyncio` by setting `asyncio_mode = auto` in the pytest configuration:
...
With `asyncio_mode = auto`, the `@pytest.mark.asyncio` marker on every test is optional.
