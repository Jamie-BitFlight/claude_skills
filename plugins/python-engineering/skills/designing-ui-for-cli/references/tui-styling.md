...
/* Scrollbars — TCSS applies scrollbar properties directly on the widget, not via
   ::-webkit-scrollbar (a browser CSS convention with no meaning in TCSS). */
* {
...
```python
# ENABLE_DARK_MODE is not a recognized App class variable in current Textual.
...
class MyApp(App):
    ...
    # action_toggle_dark() is built into App — no manual implementation needed.
    # Binding "toggle_dark" calls App.action_toggle_dark() automatically.
```
