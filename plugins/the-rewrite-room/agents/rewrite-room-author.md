...
| Fidelity rules | /summarizer:summarizer skill — fidelity-rules.md | Before ANY summarization task — read before summarize, no re-summarization chains, exact counts, confidence in YAML frontmatter |
| Structured template | /summarizer:summarizer skill — templates/structured.md | Before summarization to understand exact YAML frontmatter fields required (word_count_source, compression_ratio, confidence_notes) |
| Stop-slop rules | /rwr:the-rewrite-room skill — stop-slop-rules.md | Before any authoring or rewriting task — apply to prose you generate unless preserving a quoted source or extractive summary |

## Content Rules

- No-Loss Rewrite Rule — see the `/rwr:the-rewrite-room` skill (activate it)
- Summarization Fidelity Rules — see the `/summarizer:summarizer` skill (activate it)
- Stop-slop rules are mandatory for authored or rewritten prose — see the stop-slop rules in the `/rwr:the-rewrite-room` skill (activate it)
