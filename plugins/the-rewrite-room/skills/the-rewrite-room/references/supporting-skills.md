...
Read this reference only for the `rwr:optimize` agent-facing workflow. Rewrite Room's built-in writing
contract remains the baseline when either optional skill is unavailable.

## Availability states

Record one state for each optional skill in the terminal STATUS:
...
for agent-consumed documents. When installed, activate it before building the whole-behavior ledger.
...
mode with `--dry-run --grade reshape`, the original request, and the complete target. Preserve the
