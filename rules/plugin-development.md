...
**Automatic version bumping**: the pre-commit hook bumps and stages plugin versions when plugin content changes, so `/reload-plugins` sees branch edits without a session restart. Marketplace version synchronization is deferred to the post-merge repair PR. Do not manually edit version fields for a normal local-commit workflow — the hook includes the plugin versions in the commit.

The stock `agent-marketplace-versioner` hook owns local synchronization. Its shared GitHub Action checks PR versions against the actual base and head revisions. Branch-side derivation can still collide when sibling PRs choose the same version. After merges, `.github/workflows/bump-marketplace.yml` runs shared historical repair and synchronization, then proposes corrections in a normal PR. Corrections become effective after review and merge; the workflow does not write directly to `main`. See [marketplace versioning](../docs/marketplace-versioning.md) for the required repair credential and compatibility tools.
