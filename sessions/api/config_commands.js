...
function handleConfigCommand(args, jsonOutput = false, fromSlash = false) {
  ...
  if (section === 'show') {
    ...
    throw new Error(
      `Unknown config section: ${section}. Valid sections: phrases, git, env, features, read, write, tools, readonly, validate`,
    );
  }
}
...
function formatConfigHuman(config) {
  ...
  const lines = ['=== Sessions Configuration ===', '', 'Trigger Phrases:'];
...
function handlePhrasesCommand(args, jsonOutput = false, fromSlash = false) {
  ...
  if (action === 'list') {
    ...
    throw new Error(
      `Unknown phrases action: ${action}. Valid actions: list, add, remove, clear, show`,
    );
  }
}
  ...
  editConfig((config) => {
    if (['add', 'add_pattern'].includes(key)) {
      ...
      if (!['conventional', 'simple', 'detailed'].includes(style)) {
        ...
        throw new Error(
          `Invalid commit style: ${value}. Valid options: conventional, simple, detailed`,
        );
      }
      ...
      if (fromSlash) {
        if (value === 'auto') {
          ...
          throw new Error(
            `Invalid value '${value}' for merge\nValid options: auto (merge automatically) or ask (prompt first)\n\nUse '/sessions config git help' for more info`,
          );
        }
      } else {
        config.git_preferences.auto_merge = ['true', 'yes', '1', 'auto'].includes(
          value.toLowerCase(),
        );
      }
      ...
      if (fromSlash) {
        if (value === 'auto') {
          ...
          throw new Error(
            `Invalid value '${value}' for push\nValid options: auto (push automatically) or ask (prompt first)\n\nUse '/sessions config git help' for more info`,
          );
        }
      } else {
        config.git_preferences.auto_push = ['true', 'yes', '1', 'auto'].includes(
          value.toLowerCase(),
        );
      }
      ...
      if (fromSlash) {
        if (value === 'super') {
          ...
          throw new Error(
            `Invalid value '${value}' for repo type\nValid options: super (has submodules) or mono (single repo)\n\nUse '/sessions config git help' for more info`,
          );
        }
      } else {
        config.git_preferences.has_submodules = ['true', 'yes', '1', 'super'].includes(
          value.toLowerCase(),
        );
      }
    } else {
  ...
  if (action === 'set') {
    ...
    editConfig((config) => {
      if (['task_detection', 'auto_ultrathink', 'branch_enforcement'].includes(key)) {
        ...
        if (!validValues.includes(lowerValue)) {
          throw new Error(
            `Invalid icon_style value: ${value}. Valid values: nerd_fonts, emoji, ascii`,
          );
        }
    ...
    const oldDisplay =
      typeof currentValue === 'object' && currentValue.value !== undefined
        ...
        : currentValue;
    const newDisplay =
      typeof newValue === 'object' && newValue.value !== undefined ? newValue.value : newValue;
...
function handleReadonlyCommand(args, jsonOutput = false, _fromSlash = false) {
  /**
