...
function checkCommandArguments(parts) {
  ...
  if (['awk', 'gawk', 'mawk'].includes(cmd)) {
    ...
    if (
      script.includes('print >') ||
      script.includes('print >>') ||
      script.includes('printf >') ||
      script.includes('printf >>')
    ) {
      return false;
    }
...
function isBashReadOnly(command, _extrasafe = CONFIG.blocked_actions.extrasafe || true) {
...
if (toolName === 'Bash' && STATE.mode === Mode.NO && !STATE.flags.bypass_mode) {
  ...
  if (
    command &&
    (command.includes('sessions ') || command.includes('python -m cc_sessions.scripts.api'))
  ) {
  ...
  if (!isBashReadOnly(command)) {
    ...
    console.error(
      `[DAIC] Blocked write-like Bash command in Discussion mode. Only the user can activate implementation mode. Explain what you want to do and seek alignment and approval first.\n` +
...
if (
  ...
  if (!isBashReadOnly(command)) {
    console.error(
      '[Security] Direct modification of sessions-state.json is not allowed. ' +
...
if (STATE.mode === Mode.NO && !STATE.flags.bypass_mode) {
  if (CONFIG.blocked_actions.isToolBlocked(toolName)) {
    console.error(
      `[DAIC: Tool Blocked] You're in discussion mode. The ${toolName} tool is not allowed. You need to seek alignment first.`,
    );
    process.exit(2); // Block with feedback
  } else {
...
if (repoPath) {
  try {
    ...
    const inTask =
      STATE.current_task.submodules?.includes(submoduleName) || repoPath === PROJECT_ROOT; // Root repo - always considered in task
    ...
    // Scenario 2: Submodule is in task but on wrong branch
      ...
      console.error(
        `[Branch Mismatch] Submodule '${submoduleName}' is part of this task but is on branch '${currentBranch}' instead of '${expectedBranch}'.`,
      );
      console.error(
        `Please run: cd ${path.relative(PROJECT_ROOT, repoPath)} && git checkout ${expectedBranch}`,
      );
      process.exit(2);
    }
    // Scenario 3: Submodule not in task but already on correct branch
      ...
      console.error(
        `[Submodule Not in Task] Submodule '${submoduleName}' is on the correct branch '${expectedBranch}' but is not listed in the task file.`,
      );
      console.error(
        `Please update the task file to include '${submoduleName}' in the submodules list.`,
      );
      process.exit(2);
    }
    // Scenario 4: Submodule not in task AND on wrong branch
      ...
      console.error(
        `[Submodule Not in Task + Wrong Branch] Submodule '${submoduleName}' has two issues:`,
      );
      ...
      console.error(
        `To fix: cd ${path.relative(PROJECT_ROOT, repoPath)} && git checkout -b ${expectedBranch}`,
      );
      console.error(
        `Then update the task file to include '${submoduleName}' in the submodules list.`,
      );
      process.exit(2);
    }
  } catch (error) {
    ...
    console.error(
      `Warning: Could not verify branch for ${path.basename(repoPath)}: ${error.message}`,
    );
  }
}
//!<
